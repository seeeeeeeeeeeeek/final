...
- you open your normal thinkorswim web tab yourself
- you log in manually there
- you run the helper script once in that real tab
- stocknogs requests ticker switches through that live tab and reads selector-based DOM data back
- if needed, you can still paste selector JSON into the manual-session box as a fallback
...
3. Open your normal logged-in thinkorswim web tab.
4. Run the helper script in that tab from DevTools Console.
5. Enter a ticker and click `Analyze`.
6. Watch `Run Status` for helper heartbeat, helper event, and final result state.
7. Open `Analysis Detail` to inspect the readable summary, source path, detailed analysis, and advanced diagnostics.
8. Review `History` to confirm persistence across sessions.
...
3. Run that script from DevTools Console on the thinkorswim page.
4. Leave that tab open.
5. In stocknogs, type a ticker and click `Analyze`.
6. The helper should switch the live tab, read selectors, and report the result back.
