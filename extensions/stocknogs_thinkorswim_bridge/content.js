(function () {
  ...
  if (window.__stocknogsBridgeRunning) {
    return;
  }
  window.__stocknogsBridgeRunning = true;
  ...
  function timeframeButton(target) {
    ...
    for (const selector of selectorGroup) {
      const node = document.querySelector(selector);
      if (node) {
        return { node, selector, controlKind: TIMEFRAME_TARGET_CONTROL[normalized] || "aggregation" };
      }
    }
  ...
  function timeframeOption(target) {
    ...
    const options = Array.from(
      document.querySelectorAll(TIMEFRAME_OPTION_SELECTORS.join(","))
    );
  ...
  async function switchSymbol(symbol) {
    ...
    if (typeof inputResult.node.setSelectionRange === "function") {
      try {
        ...
        // Ignore browsers that do not support selection on this input.
      }
    }
  ...
  async function reportDebug(event, payload) {
    await send({
      type: "bridge-report-debug",
      event,
      payload: {
        page_url: window.location.href,
        page_title: document.title || null,
  ...
  async function tick() {
    ...
    try {
      ...
      if (!command || !command.symbol || command.id === lastCommandId) {
        return;
      }
      busy = true;
      ...
      await reportDebug("switch_started", {
        ...
        message: `Extension bridge received ${command.symbol} and is trying to switch the live tab.`,
      });
      const searchSelector = await switchSymbol(command.symbol);
      await new Promise((resolve) => window.setTimeout(resolve, POST_SWITCH_WAIT_MS));
      await reportDebug("switch_finished", {
        symbol: command.symbol,
        message: `Extension bridge switched the live tab to ${command.symbol} and is reading selectors.`,
        selector: searchSelector,
      });
      const payload = collectPayload(command.symbol, searchSelector);
      const reportResponse = await send({
        type: "bridge-report-payload",
        commandId: command.id,
        payload,
      });
      await reportDebug("report_finished", {
        symbol: command.symbol,
        message: `Extension bridge reported selector data for ${command.symbol}.`,
        latest_scan_id: reportResponse?.response?.record?.scan_id || null,
      });
    } catch (error) {
      await send({
        type: "bridge-command-failed",
        commandId: lastCommandId || null,
        symbol: null,
        pageUrl: window.location.href,
        pageTitle: document.title || null,
        error: error?.message || String(error),
        message: `Extension bridge hit an in-tab error: ${error?.message || String(error)}`,
      });
      console.error("[stocknogs bridge] command failed", error);
    } finally {
      busy = false;
    }
  }
  ...
  window.__stocknogsBridgeSelectors = {
    aggregationButtonSelectors: AGGREGATION_BUTTON_SELECTORS,
    rangeButtonSelectors: RANGE_BUTTON_SELECTORS,
    timeframeOptionSelectors: TIMEFRAME_OPTION_SELECTORS,
  };
  tick();
})();
