...
<body>
  <div class="app">
    <aside class="sidebar">
      ...
      <div class="tagline">A local breakout-analysis workspace powered by your real thinkorswim web tab plus a manual in-page helper.</div>
      ...
      <section id="live" class="page">
        ...
        <div class="grid two-col">
          <div class="card">
            ...
            <div class="helper-copy">Use thinkorswim web as the only live source. Preferred flow: keep your real logged-in browser tab open, run the helper in that tab once, and then let stocknogs request ticker switches there.</div>
            ...
            <div style="margin-top:16px;">
              ...
              <div class="helper-copy">Manual fallback only. The preferred mode is the running helper script inside your real thinkorswim tab. If needed, you can still paste a selector-based JSON payload here and submit it.</div>
  ...
  <script>
    ...
    function renderSourceProgramStatus(settings) {
      ...
      return `<div class="section-title">Source Program</div><div class="helper-copy">${escapeHtml(program.message || 'thinkorswim web is the only active source.')}</div><div class="summary-line"><strong>Status</strong><span>${escapeHtml(program.active ? 'Active' : 'Paused')}</span></div><div class="summary-line"><strong>Current mode</strong><span>${escapeHtml(program.label || 'thinkorswim web')}</span></div><div class="summary-line"><strong>Browser running</strong><span>${escapeHtml(browserStatus.running ? 'Yes' : 'No')}</span></div><div class="summary-line"><strong>Profile dir</strong><span>${escapeHtml(browserStatus.profile_dir || 'Not set')}</span></div><div class="summary-line"><strong>Current page</strong><span>${escapeHtml(browserStatus.page_title || browserStatus.current_url || 'Not available')}</span></div><div class="summary-line"><strong>Helper script</strong><span>${escapeHtml(program.manual_session_helper?.path || 'scripts/thinkorswim_manual_session_helper.js')}</span></div><div class="summary-line"><strong>Helper heartbeat</strong><span>${escapeHtml(helperStatus.last_seen_at || 'Not seen yet')}</span></div><div class="summary-line"><strong>Helper event</strong><span>${escapeHtml(helperStatus.last_event || 'No helper activity yet')}</span></div>${helperStatus.last_error ? `<div class="summary-line"><strong>Helper error</strong><span>${escapeHtml(helperStatus.last_error)}</span></div>` : ''}<div class="summary-line"><strong>Archived integrations</strong><span>${escapeHtml(archived.join(', ') || 'None')}</span></div><div class="helper-copy">Best flow: run the helper inside your real logged-in tab once. Then when you request a symbol in stocknogs, that tab will switch to it and report the updated DOM values back.</div>`;
    }
    ...
    async function renderHome() {
      ...
      if (!records.length) {
        homeBody.innerHTML = `<div class="hero"><h2>No live records yet</h2><p class="muted">Open your normal thinkorswim web tab, log in there, run the helper script once in that tab, and then use Live Analysis to request symbols from that live session.</p><div class="button-row"><button class="action" id="home-open-live">Open Live Analysis</button><button class="ghost" id="home-open-settings">Open Strategy Settings</button></div></div>`;
